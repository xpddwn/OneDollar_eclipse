...
public abstract class ImageWorker {
	...
	private static final String INTERNET_RAQULAR = "^http[s]?.*";
	...
	public void loadImage(Object data, ImageView imageView,
			OnBitmapDownload callBack, Bitmap defaultBitmap) {
		if (data == null) {
			return;
		}
		String url = String.valueOf(data);
		if (matchInertetAddress(url)) {
		} else {
			url = Constants.SERVER_IMG_ADDRESS + url;
		}
		...
		if (value != null) {
			...
			if (cancelPotentialWork(url, imageView)) {
				...
				final BitmapWorkerTask task = new BitmapWorkerTask(url,
						imageView, callBack);
				final AsyncDrawable asyncDrawable = new AsyncDrawable(
						mResources, mLoadingBitmap, task);
				if (callBack != null) {
					callBack.onFinish(imageView, asyncDrawable);
				} else {
				...
				// pulled from the
				// framework and slightly modified. Refer to the docs at the top
				// of
				// the class
	...
	private class BitmapWorkerTask extends
		...
		private OnBitmapDownload mcallBack = null;

		public BitmapWorkerTask(Object data, ImageView imageView,
				OnBitmapDownload callBack) {
			mData = data;
		...
		protected BitmapDrawable doInBackground(Void... params) {
			...
			if (BuildConfig.DEBUG && LOG) {
				...
				Log.d(TAG, "doInBackground - finished work : drawable, ",
						drawable);
			}
		...
		protected void onPostExecute(BitmapDrawable value) {
			...
			if (value != null && imageView != null) {
				...
				setImageDrawable(imageView, value, mcallBack);
			}
	...
	public static boolean matchInertetAddress(String url) {
		Pattern pattern = Pattern.compile(INTERNET_RAQULAR);
		Matcher matcher = pattern.matcher(url);
		return matcher.matches();
	}

	public static interface OnBitmapDownload {
		public void onFinish(ImageView imageView, Drawable drawable);
	}
}
